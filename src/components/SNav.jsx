    ...
    <nav className="fixed left-0 top-0 z-50 w-full border-b border-border bg-background/80 backdrop-blur-md transition-colors duration-300">
      ...
      {isMenuOpen && (
        <div className="animate-in slide-in-from-top-5 absolute left-0 top-16 w-full border-b border-border bg-background shadow-lg md:hidden">
          <div className="flex max-h-[85vh] flex-col space-y-4 overflow-y-auto p-4">
            {NAV_LINKS.map((link) => (
              <a
                key={link.href}
                href={link.href}
                onClick={() => setIsMenuOpen(false)}
                className="border-b border-border/50 py-2 text-base font-medium text-foreground transition-colors hover:text-primary"
              >
                {link.label}
              </a>
            ))}
            ...
            <div className="mt-2 space-y-3 rounded-lg bg-muted/30 p-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 rounded-md border border-border bg-background p-1">
                  <button onClick={decreaseFont} disabled={fontSize <= 85} className="rounded p-2 hover:bg-muted">
                    <Minus size={16} />
                  </button>
                  <span className="w-8 text-center font-mono text-xs">{fontSize}%</span>
                  <button onClick={increaseFont} disabled={fontSize >= 125} className="rounded p-2 hover:bg-muted">
                    <Plus size={16} />
                  </button>
                </div>
              </div>
              ...
              <button
                onClick={toggleFont}
                className="flex w-full items-center justify-between rounded-md p-2 transition-colors hover:bg-background"
              >
                <span className="text-sm text-foreground">Fonte Dislexia</span>
                <div
                  className={`rounded p-1 ${fontDyslexic ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                >
                  <Type size={16} />
                </div>
              </button>
              ...
              >
                ...
                <div
                  className={`rounded p-1 ${theme === "dark" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                >
