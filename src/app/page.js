  ...
  return (
    <main className="flex min-h-screen w-full flex-col items-center bg-background pt-20 transition-colors duration-300">
      ...
      <div className="flex w-full max-w-[960px] flex-1 flex-col gap-16 px-4">
        <section aria-labelledby="hero-title" className="mt-8 flex flex-col items-center space-y-8 text-center">
          <div className="max-w-3xl space-y-4">
            ...
            <p className="mx-auto max-w-2xl text-lg font-normal leading-relaxed text-muted-foreground md:text-xl">
              Entenda como nossa ferramenta ajuda a criar experiências digitais mais inclusivas através de personas e
        ...
        <SectionCard title="O que é Neurodivergência?" id="O_que">
          <p className="text-base font-normal leading-relaxed text-muted-foreground md:text-lg">
            Neurodivergência é um termo que descreve variações naturais no cérebro humano em relação à sociabilidade,
            aprendizagem, atenção, humor e outras funções mentais. Nosso objetivo é desmistificar o conceito e promover
            a importância da acessibilidade digital para criar ambientes online onde todos possam navegar
            confortavelmente.
          </p>
        </SectionCard>
        ...
        <SectionCard id="personas" title="Nossas Personas">
          <p className="mb-6 text-base font-normal leading-relaxed text-muted-foreground md:text-lg">
            Para avaliar a acessibilidade de forma eficaz, utilizamos personas baseadas em pesquisas e entrevistas. Elas
            representam diferentes perfis neurodivergentes.
          </p>

          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {PERSONAS.map((persona) => (
              <InfoCard key={persona.id} title={persona.title} description={persona.description} icon={persona.icon} />
            ))}
          </div>
        </SectionCard>
        ...
        <SectionCard id="atores" title="Atores do Sistema">
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {ACTORS.map((actor) => (
              <InfoCard key={actor.id} title={actor.title} description={actor.description} icon={actor.icon} />
            ))}
          </div>
        </SectionCard>

        <SectionCard id="funcionalidades" title="Funcionalidades Principais">
          <p className="mb-8 text-muted-foreground">Recursos projetados para avaliações completas e acessíveis.</p>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
            {FEATURES.map((feature) => (
              <InfoCard key={feature.id} title={feature.title} description={feature.description} icon={feature.icon} />
            ))}
          </div>
        </SectionCard>
        ...
        <SectionCard id="Likert" title="Como funciona a Avaliação">
          <p className="mb-6 text-base font-normal leading-relaxed text-muted-foreground md:text-lg">
            Nossa avaliação utiliza uma escala Likert, um método comprovado para medir atitudes e percepções. Cada ponto
            da escala tem um significado claro para garantir que suas respostas reflitam com precisão sua experiência.
          </p>
      ...
      <footer className="mt-16 w-full border-t border-border bg-card py-8">
        <div className="flex justify-center">
          ...
          >
            <Github size={20} className="transition-transform duration-200 group-hover:-translate-y-0.5" />
