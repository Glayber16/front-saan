...
export default function VisualizarPage() {
    ...
    const exitEvaluation = () => {
        if (confirm("Deseja sair da avaliação? O progresso não salvo será perdido (exceto o estado atual da memória do navegador).")) {
                ...
                {loading ? (
                    ...
                    activeAssignment ? (
                                ...
                                <div className="animate-fade-in">
                                    ...
                                    <div className="mb-8">
                                        <div className="flex justify-between text-sm text-muted-foreground mb-2">
                                            <span>Progresso: Passo {currentPage + 1} de {groupsOrder.length}</span>
                                    ...
                                    {/* Group Title */}
                                    <div className="card-calm mb-8 p-6 bg-primary/5 border-primary/20">
                                        <h2 className="text-xl font-bold text-foreground mb-2">{currentGroup}</h2>
                                    ...
                                    <div className="space-y-8">
                                        {currentQs.map((q, idx) => {
                                            ...
                                            return (
                                                <div id={`question-${q.id}`} key={q.id} className="rounded-lg border border-border bg-card p-6 transition-all duration-300">
                                                    <div className="mb-4 text-lg font-medium">
                                                        <span className="text-muted-foreground mr-2">#{absIdx + 1}</span>
                                                        {q.text}
                                                    </div>
                                                    {q.example && (
                                                        <div className="mb-6 rounded-md bg-muted p-4 text-sm text-muted-foreground italic border-l-4 border-primary/50">
                                                            <strong className="block text-primary not-italic mb-1">Contexto/Exemplo:</strong>
                                                            {q.example}
                                                        </div>
                                                    )}
                                                    ...
                                                    <div className="grid grid-cols-5 gap-2 md:gap-4">
                                                        {[1, 2, 3, 4, 5].map((val) => {
                                                            const isSelected = answers[`${activeAssignment.applicationId}_${activeAssignment.form.id}`]?.[q.id] === val;
                                                            return (
                                                                <button
                                                                    key={val}
                                                                    type="button"
                                                                    onClick={() => handleSelectOption(activeAssignment.applicationId, activeAssignment.form.id, q.id, val)}
                                                                    className={`flex flex-col items-center justify-center rounded-lg border p-3 text-center transition-all hover:scale-105 active:scale-95 ${isSelected
                                                                        ? "border-primary bg-primary text-primary-foreground shadow-lg shadow-primary/25"
                                                                        : "border-border bg-background text-muted-foreground hover:border-primary hover:text-foreground"
                                                                        }`}
                                                                >
                                                                    <span className="text-xl font-black mb-1">{val}</span>
                                                                    <span className="hidden text-[10px] uppercase tracking-wider md:inline-block font-semibold opacity-80">
                                                                        {val === 1 && "Discordo"}
                                                                        {val === 5 && "Concordo"}
                                                                    </span>
                                                                </button>
                                        ...
                                        >
                                            {submitting ? (
                                                ...
                                                currentPage === groupsOrder.length - 1 ? (
                                                    <span className="flex items-center gap-2">Finalizar <Send size={18} /></span>
                                                ) : (
                                                    "Próxima Seção"
                                                )
                                            )}
                        ...
                        assignments.length === 0 ? (
                            <div className="rounded-lg border border-dashed border-border p-16 text-center text-muted-foreground bg-muted/30">
                                <ClipboardList size={64} className="mx-auto mb-6 opacity-30" />
                                <h3 className="text-xl font-semibold mb-2">Tudo certo por aqui!</h3>
                            ...
                            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                {assignments.map((assignment) => (
                                    <div
                                        ...
                                        className="card-calm p-0 overflow-hidden cursor-pointer group hover:border-primary transition-colors"
                                    ...
                                    >
                                        <div className="p-6">
                                            ...
                                            <p className="text-sm text-muted-foreground mb-4">
                                                Formulário: {assignment.form.title}
                                            </p>
                                        </div>
                                ...
                                <div className="space-y-8">
                                    {form.questions && form.questions.map((q, idx) => (
                                        <div key={q.id} className="rounded-lg border border-border bg-card/50 p-6 opacity-80">
                                            ...
                                            {q.example && (
                                                <div className="mb-4 rounded bg-muted/50 p-3 text-sm text-muted-foreground italic border-l-4 border-primary/20">
                                                    Exemplo: {q.example}
                                                </div>
                                            )}
                                            <div className="grid grid-cols-5 gap-2">
                                                {[1, 2, 3, 4, 5].map((val) => (
                                                    <div key={val} className="flex flex-col items-center justify-center rounded-md border border-border p-3 text-center text-xs text-muted-foreground bg-muted/30">
